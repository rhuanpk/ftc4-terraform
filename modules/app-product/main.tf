...
resource "kubernetes_service" "app_product_service_load_balancer" {
  metadata {
    name      = "app-product-service-loadbalancer"
    namespace = var.namespace
  }
...
resource "kubernetes_service" "app_product" {
  metadata {
    name      = var.name
    namespace = var.namespace
  }
...
module "app_product_autoscaling" {
  source              = "./autoscaling"
  name                = "${var.name}-hpa"
  namespace           = var.namespace

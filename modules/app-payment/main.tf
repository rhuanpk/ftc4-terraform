...
resource "kubernetes_service" "app_payment_service_load_balancer" {
  metadata {
    name      = "app-payment-service-loadbalancer"
    namespace = var.namespace
  }
...
resource "kubernetes_service" "app_payment" {
  metadata {
    name      = var.name
    namespace = var.namespace
  }
...
module "app_payment_autoscaling" {
  source              = "./autoscaling"
  name                = "${var.name}-hpa"
  namespace           = var.namespace
